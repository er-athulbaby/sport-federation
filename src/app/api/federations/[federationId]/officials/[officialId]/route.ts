...
export async function PATCH(
  ...
  values.push(officialId, federationId);
  try {
    ...
    if (result.rows.length === 0) return errorResponse("Official not found", 404);
...
export async function DELETE(
  ...
  const result = await pool.query(
    "DELETE FROM officials WHERE id = $1 AND federation_id = $2 RETURNING id",
