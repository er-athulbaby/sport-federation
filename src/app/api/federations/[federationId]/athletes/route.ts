...
export async function GET(
  ...
  const result = await pool.query(
    `SELECT id, full_name_en, full_name_ar, gender, dob, height_cm, weight_kg,
...
export async function POST(
  ...
  if (!full_name_en || !full_name_ar || !gender || !passport_number || !passport_expiry_date) {
    return errorResponse(
      "full_name_en, full_name_ar, gender, passport_number, and passport_expiry_date are required"
    );
  }
    ...
    const result = await pool.query(
      `INSERT INTO athletes (federation_id, full_name_en, full_name_ar, gender, dob,
       ...
       RETURNING id, full_name_en, full_name_ar, gender, dob, height_cm, weight_kg,
                 passport_number, passport_expiry_date, photo_url, tshirt_size, suit_size,
                 created_by, created_at, updated_at`,
      [federationId, full_name_en, full_name_ar, gender, dob ?? null,
       height_cm ?? null, weight_kg ?? null, passport_number, passport_expiry_date,
